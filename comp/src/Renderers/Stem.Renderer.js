...
function CreateStems(notes, divisions, staff, measure, camera) {
    ...
    divisions.forEach((div, i) => {
        ...
        if (stemDir === StemDirection.Up) {
            stem.Bounds.y = divNotes[divNotes.length - 1].Bounds.y + 2.5 + camera.y;
            stem.Bounds.height = hNote.Bounds.y - divNotes[divNotes.length - 1].Bounds.y - 35;
        }
        else {
            stem.Bounds.y = divNotes[0].Bounds.y + 2.5 + camera.y;
        ...
        if (StemToCenter(stemDir, lowestLine, highestLine, staffRelativeMid)) {
            stem.Bounds.height = staffMidLinePos - stem.Bounds.y + camera.y + measure.Bounds.y;
        }
        if (shouldBeam) {
