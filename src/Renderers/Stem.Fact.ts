...
function CreateStems(
  ...
  console.log(
    "Grace notes: " + notes[0].filter((n: Note) => n.Grace === false).length,
  );
  divisions.forEach((div: Division, i: number) => {
    div.Subdivisions.forEach((subDiv: Subdivision) => {
      const beamAlt = i * (10 / divisions.length - 1);
      const divNotes = notes[i];
      const isGraceStem = subDiv.Type === SubdivisionType.GRACE_NOTE;
