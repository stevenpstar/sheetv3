...
function RenderDebugOld(
  ...
  // note details
  const selectedNotes = measure.Voices[measure.ActiveVoice].Notes.filter(
    (n) => n.Selected,
  );
  ...
  context.strokeRect(
    measure.Bounds.x + measure.XOffset + camera.x,
...
function debugGetDurationName(
  duration: number,
  alpha: number = 255,
): debugValueProperties {
  let name = "";
  let g: number = 150;
  let r: number = 0;
  let b: number = 0;
