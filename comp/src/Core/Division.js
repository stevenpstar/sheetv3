...
function CreateDivisions(msr, notes, staff, voice) {
    const divisions = [];
    let nextBeat = 0;
    let runningValue = 0;
    notes.sort((a, b) => {
...
function GenerateMissingBeatDivisions(msr, divisions, staff, voice) {
...
function GetDivisionGroups(msr, staff) {
    ...
    mDivs.forEach((div) => {
        ...
        if (divNotes.length > 0) {
            divGroups.DivGroups.push(CreateDivisionGroup(divs, notes, staff, crossStaff));
        }
    });
    mDivs.forEach((div, i) => {
        if (div.Staff !== staff) {
...
function CreateDivisionGroup(divs, notes, staff, crossStaff) {
