...
import { BeatDistribution } from "../Core/Measure.js";
...
function RenderStem(ctx: CanvasRenderingContext2D,
                    notes: Note[],
                   beatD: { startNumber: number, value: number, bounds: Bounds },
                   cam: Camera): void {
  ...
  // pixel
    ctx.fillRect(Math.floor(beatD.bounds.x + 18 + xBuffer) + cam.x, (startPos - 5) + cam.y,
               2, -diff);
...
function renderLedgerLines(
  ...
  renderProps: RenderProperties): void {
    ...
    const bdNotes = notes.filter((note: Note) => note.Beat === division.startNumber);
    bdNotes.sort((a: Note, b: Note) => {
      return a.Line - b.Line;
    });
