...
import { GetStaffHeightUntil } from "../Core/Staff.js";
...
function RenderTupletAnnotation(renderProps, coords, count, theme) {
    ...
    RenderSymbol(renderProps, GetTupletGlyph(count), coords.x1 + width / 2 - 7, coords.y1 - 5, theme, false);
}
...
function RenderTuplets(renderProps, divisions, notes, staff, msr, theme) {
    ...
    divs.forEach((div, i) => {
        ...
        if (!notesInDiv[0].Tuple) {
            if (foundTuplet) {
                foundTuplet = false;
                RenderTupletAnnotation(renderProps, { x1: tupleX, y1: tupleY, x2: tupleXEnd, y2: tupleY }, tupleCount.toString(), theme);
                tupleX = 0;
                tupleXEnd = 0;
                tupleY = 0;
                tupleCount = 0;
...
function RenderTies(renderProps, divisions, notes, staff, msr) {
    ...
    divs.forEach((div, i) => {
        ...
        divNotes.forEach((note) => {
            ...
            const tiedTo = nextDivNotes.find((n) => n.Line === note.Line && n.Tied && n.Beat <= n.TiedEnd);
            if (tiedTo === undefined) {
            ...
            context.quadraticCurveTo(x1 + noteXBuffer / 2 + (x2 - x1) / 2, y1 + curveOffset, x2 + noteXBuffer / 2, y2 + curveStartOffset);
            context.stroke();
        });
    });
}
...
function RenderStemRevise(renderProps, notes, divisions, staff, msr, beamDir, theme, colour) {
    ...
    let beamStartX = 0;
    let beamEndX = 0;
    const middleLine = GetStaffHeightUntil(msr.Staves, staff) / 5 + 15;
    const yLineBuffer = msr.Staves.find((s) => s.Num === staff).TopLine;
    ...
    // TODO: Beam Direction
    if (middleLine - highestLine + staffTopLine <
        lowestLine - middleLine - staffTopLine) {
        stemDirection = StemDirection.Up;
        stemEndY =
            divisions[0].Bounds.y +
                (highestLine - yLineBuffer) * lineHeight -
                35 +
                camera.y;
        ...
        stemEndY =
            divisions[0].Bounds.y +
                (lowestLine - yLineBuffer) * lineHeight +
                35 +
                camera.y;
...
function renderLedgerLines(notes, division, renderProps, staff, msr, theme, colour) {
    ...
    const bdNotes = notes.filter((note) => note.Beat === division.Beat && note.Staff === division.Staff);
    const midLine = 15;
    if (bdNotes.length === 0) {
        return;
    }
