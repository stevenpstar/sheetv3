...
function RenderNote(note, renderProps, Bounds, selected, flipNote, stemDir, theme, colour = "black") {
    ...
    switch (note.Duration) {
        ...
        case 0.25:
            ...
            if (note.Ghost) {
                RenderScaledSymbol(renderProps, NoteHeads.crotchet, x, y, theme, selected, Math.floor(stdFontSize * 0.6));
            }
            else {
...
function RenderTies(renderProps, divisions, notes, staff, msr) {
    ...
    divs.forEach((div, i) => {
        ...
        divNotes.forEach((note) => {
            ...
            const y1 = msr.GetNotePositionOnLine(note.Line, note.Staff) + camera.y - 4;
